...
check_astro_port() {
  ...
  # Port is derived from SITE_URL env var at runtime — check that the config
  # references astroPort or server.port, not that a literal number exists.
  if grep -qE "port.*astroPort|server.*port" "$file"; then
    pass "astro.config.mjs derives server.port from SITE_URL env var"
  else
    fail "astro.config.mjs does not set a server.port"
  fi
}
...
check_no_hardcoded_ports() {
  # Source code should never have hardcoded localhost port fallbacks.
  # Default ports belong only in .env.template — init-project.sh sets per-project ports.
  local violations
  violations="$(grep -rn \
    "localhost:[0-9]\{4,\}" \
    "$ROOT/templates" \
    --include="*.ts" --include="*.tsx" --include="*.mjs" --include="*.js" --include="*.astro" \
  ...
  if [[ -z "$violations" ]]; then
    pass "No hardcoded localhost ports in template source"
  else
    fail "Hardcoded localhost ports found (use env vars instead):"
    echo "$violations" | head -10 | while IFS= read -r line; do
      echo "    $line"
    done
  fi
}
...
# Read per-project ports from .ports (generated by init-project.sh) or .env.local
if [[ -f "$ROOT/.ports" ]]; then
  source "$ROOT/.ports"
  check_port "${ASTRO:-4400}" "Astro"
  check_port "${NEXTJS:-3100}" "Next.js / Payload"
  check_port "${TWENTY:-3001}" "Twenty CRM"
elif [[ -f "$ROOT/templates/astro-site/.env.local" ]]; then
  _port_from_url() { echo "$1" | grep -oE ':[0-9]+' | tr -d ':' | head -1; }
  _astro_port="$(_port_from_url "$(grep '^SITE_URL=' "$ROOT/templates/astro-site/.env.local" | cut -d= -f2-)")"
  _nextjs_port="$(_port_from_url "$(grep '^NEXT_PUBLIC_SERVER_URL=' "$ROOT/templates/next-app/.env.local" | cut -d= -f2-)")"
  _twenty_port="$(_port_from_url "$(grep '^TWENTY_API_URL=' "$ROOT/templates/next-app/.env.local" | cut -d= -f2-)")"
  check_port "${_astro_port:-4400}" "Astro"
  check_port "${_nextjs_port:-3100}" "Next.js / Payload"
  check_port "${_twenty_port:-3001}" "Twenty CRM"
else
  warn "No .ports or .env.local found — skipping port checks (run init-project.sh first)"
  SKIPPED=$((SKIPPED + 3))
fi
